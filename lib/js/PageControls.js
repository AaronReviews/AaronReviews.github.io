...
// the area containing the title elements
const title = $('.title');
// jennys name
const header = $('.header');
// jennys contact info
const details = $('.details');
// the revolving content box
...
function start() {
    ...
    tlstart.to(
        [SCENE.renderer.domElement, SCENE.container],
    ...
    tlstart.from(
        ...
        SCENE.spotLight, 
        SCENE.backLight], 
        { duration: 2, power: 0, stagger: 0.3, delay: -1.5 });
    tlstart.from(
        [title, mainBox], 
        { duration: 1.5, opacity: 0, y: "-50", ease: 'back', stagger: 0.3, delay: -1 });
    tlstart.to(
        header, 
        { duration: 1.5, letterSpacing: -4, ease: 'back', autoRound: false });
    tlstart.to(
        title, 
        { duration: 1, x: -225, delay: -1.2 });
    tlstart.to(
        details, 
        { duration: 1, display: 'inline-block', opacity: 1, x: 5, delay: -0.5 });
